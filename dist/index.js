...
function cliIntroText() {
    console.log(boxen(chalk.whiteBright(figlet.textSync("Employee\nManager", {
        font: "Standard",
...
function createDisplayTable(object) {
...
function getDepartmentId(department, department_list) {
...
function getTableId(manager, manager_list) {
...
async function addRole() {
    ...
    await inquirer
        ...
        fs.readFile("./db_queries/add_role.sql", "utf8", (err, data) => {
            ...
            const departmentId = getDepartmentId(answers.department, departmentList[1]);
            pool.query(data, [answers.name, answers.salary, departmentId], (err, _res) => {
                if (err) {
                    console.error(err);
                    return;
                }
...
async function addEmployee() {
    ...
    await inquirer
        ...
        fs.readFile("./db_queries/add_employee.sql", "utf8", (err, data) => {
            ...
            pool.query(data, [answers.first_name, answers.last_name, roleId, managerId], (err, _res) => {
                if (err) {
                    console.error(err);
                    return;
                }
...
async function updateEmployeeRole() {
    ...
    await inquirer
        ...
        fs.readFile("./db_queries/update_employee_role.sql", "utf8", (err, data) => {
            if (err) {
                console.error(err);
                return;
            }
...
async function listManagers(listManagers = listEmployees()) {
    const mlist = await listManagers;
    let managerList = mlist[0];
    let updateManagerList = mlist[1];
...
async function employeePromptQuestions() {
    await inquirer
        .prompt([
        {
            name: "action",
